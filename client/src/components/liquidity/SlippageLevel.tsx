...
type Props = {
  ...
  updateSlippageLevel: (level: number) => any;
};
export const SlippageLevel = ({
  slippageLevel,
  updateSlippageLevel,
}: Props) => {
  const [mode, setMode] = useState<"view" | "edit">("view");
  const [tLevel, setTLevel] = useState(slippageLevel);
    ...
    <div className="flex items-center ">
      <input
        ...
        onChange={(e: any) => {
          ...
          formattedValue =
            Math.min(Number(formattedValue), 50) === 50
              ? "50.00"
